            ...
            <div className="overflow-hidden py-4">
                <motion.div className="flex flex-col gap-4 pb-4" animate={{ translateY: ['0%', '-50%'] }} transition={{ duration, repeat: Infinity, ease: 'linear', repeatType: 'loop' }}>
                    {[...testimonials, ...testimonials].map((item, index) => (
                        <Card key={index} className="w-[280px] sm:w-[320px] p-6 bg-card border-border/50 shadow-md">
                            <p className="text-sm leading-relaxed text-muted-foreground mb-4">&ldquo;{item.text}&rdquo;</p>
                            <div className="flex items-center gap-3">
                                <img src={item.image} alt={item.name} className="size-10 rounded-full object-cover" />
                                <div>
    ...
    return (
        <section id="testimonials" className="bg-background my-20 relative">
            <div className="container z-10 mx-auto">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1 }} viewport={{ once: true }} className="flex flex-col items-center justify-center max-w-[540px] mx-auto">
                    <div className="flex justify-center">
                        <div className="border py-1 px-4 rounded-lg">Client Stories</div>
                    </div>
